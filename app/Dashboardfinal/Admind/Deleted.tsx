...
const Deleted = ({ table, type, data, id }: DeletedProps) => {
  const size = type === "add" ? "w-8 h-8" :type==="filter"? "w-8 h-8":type==="shared image"?"w-8 h-8":"w-7 h-7";
 ...
 const[open,setopen]=useState(false);
  ...
  return (
    <div>
      <button className={`${size} iconcolor flex items-center justify-center rounded-full  ${back}`} onClick={()=>setopen(true)}>
        <Image src={`/${type}.png`} alt={type} height={14} width={14} />
      </button>
       {open &&(  <div className='w-screen h-screen absolute left-0 top-0  opacity Z-50 flex items-center justify-center'>
